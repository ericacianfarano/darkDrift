...
def measure_visual_areas(sign_thresh, amplitude, mask, pixel_size_mm=1.0, min_area=50, k_amp_threshold=1.0):
    ...
    # we want to take the maximum negative area, and if its under the min area (filter), then we set to 0
    if neg_areas:
    ...
    #
    # plt.figure()
    # mid_row = cropped.shape[0] // 2
    # mid_col = cropped.shape[1] // 2
    # horizontal_line = cropped[mid_row, :]
    # vertical_line = cropped[:, mid_col]
    # plt.plot(horizontal_line, c='black', label='horizontal')
    # plt.plot(vertical_line, c='red', label='vertical')
    # plt.legend()
    # plt.show()

    # from skimage.measure import profile_line
    # center = np.array(cropped.shape) // 2
    # length = min(cropped.shape) // 2  # half-diagonal
    #
    # angles = np.linspace(0, np.pi, 8)  # 8 different directions
    #
    # plt.figure()
    #
    # for theta in angles:
    ...
    #
    #     # make sure start/end are inside the array
    #     start = np.clip(start, [0, 0], np.array(cropped.shape) - 1)
    #     end = np.clip(end, [0, 0], np.array(cropped.shape) - 1)
    #
    #     # line = profile_line(cropped, start, end, mode='constant', cval=np.nan)
    #     line = profile_line(cropped, start, end, mode='constant', cval=np.nan, order=0)
    #     plt.plot(line, alpha=0.5)
    #
    # plt.title("Line profiles at multiple angles")
    # plt.show()

    # import numpy as np
    # from skimage.measure import profile_line
    # center = np.array(combined.shape) // 2
    # angles = np.linspace(0, np.pi, 20)
    # plt.figure()
    # for theta in angles:
    #     end = center + 200 * np.array([np.sin(theta), np.cos(theta)])
    #     line = profile_line(combined, center, end)
    #     plt.plot(line, alpha=0.4)
    # plt.title("Radial line profiles")
    # plt.show()

    # we want to take the maximum positive area, and if its under the min area (filter), then we set to 0
    ...
    return total_area_mm2, largest_neg_area_mm2, masked_amp_mean, largest_pos_area_mm2, masked
