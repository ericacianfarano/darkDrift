...
#
#
# def compute_spatial_autocorrelation(sign_map):
#     # Remove mean (important to center the signal)
#     sign_map_centered = sign_map - np.mean(sign_map)
#
#     # Compute 2D autocorrelation
#     autocorr = correlate2d(sign_map_centered, sign_map_centered, mode='full', boundary='symm')
#
#     return autocorr
#
# def plot_autocorrelation(autocorr, title='Spatial Autocorrelation'):
#     plt.figure(figsize=(6, 5))
#     plt.imshow(autocorr, vmax = 10000, cmap='viridis', origin='lower')
#     plt.title(title)
...
class RetinotopicMap():
    ...
    def _get_patch_map(self):
        ...
        self.amp_threshold = np.mean(self.masked_amp_map) + 1.5 * np.std(self.masked_amp_map)
        ...
        total_area, largest_neg_area, masked_amp_mean, largest_pos_area, self.masked_patch_map = measure_visual_areas(self.sign_thresh, self.masked_amp_map, self.mask, min_area = self.min_area, k_amp_threshold = 1)
        ...
        total_visual_area[self.animal] = 100*total_area / np.sum((self.amp_mask & self.mask))
        mean_amplitude[self.animal] = masked_amp_mean
        largest_neg_patch_area[self.animal] = 100*largest_neg_area/ np.sum((self.amp_mask & self.mask))
        largest_pos_patch_area[self.animal] = 100*largest_pos_area/ np.sum((self.amp_mask & self.mask))

        print(f"Total visual area = {total_area:.2f} pixels")
        print(f"Largest negative patch area = {largest_neg_area:.2f} pixels")
        print(f"Largest positive patch area = {largest_pos_area:.2f} pixels")
    ...
    def plot(self):
        ...
        cbar_sign = pl.colorbar(im, ax=self.ax[0], fraction=0.04)
        # print(np.nanmin(sign_map), np.nanmax(sign_map), np.isnan(sign_map).mean())
        ...
        self.ax[2].imshow(rotate_image(self.patch, self.rotateMap), cmap=pl.cm.gray, norm = norm)#, vmin = -75, vmax = 85)
        cbar_azi = pl.colorbar(azi_contour, ax=self.ax[2], fraction = 0.04)
        # azi_contour = self.ax[2].contourf(
        #     np.where(self.mask, rotate_image(azimuth_map, self.rotateMap), np.nan),
        #     cmap=pl.cm.jet,
        #     levels=np.linspace(-75, 85, 10),
        #     zorder=-1
        # )
        # self.ax[2].imshow(
        ...
        elev_levels = np.linspace(-1.25, 1.5, 15)
        ...
        tick_vals = cbar_azi.get_ticks()
        cbar_azi.set_ticks(tick_vals)
        cbar_azi.set_ticklabels(np.round(tick_vals * deg_per_phase_width, 2))  # Convert to degrees
        ...
        self.ax[3].imshow(rotate_image(self.patch, self.rotateMap), cmap=pl.cm.gray, norm = norm)#, vmin = -43, vmax = 65)
        # elev_contour = self.ax[3].contourf(
        #     np.where(self.mask, rotate_image(elevation_map, self.rotateMap), np.nan),
        #     cmap=pl.cm.jet,
        #     levels=np.linspace(-43, 65, 10),
        #     zorder=-1
        # )
        # self.ax[3].imshow(
        #     rotate_image(self.patch, self.rotateMap),
        ...
        cbar_el = pl.colorbar(elev_contour, ax=self.ax[3], fraction = 0.04)

        # Convert colorbar ticks to visual degrees
        tick_vals = cbar_el.get_ticks()
        cbar_el.set_ticks(tick_vals)
        cbar_el.set_ticklabels(np.round(tick_vals * deg_per_phase_height, 2))  # Convert to degrees
        ...
        plt.savefig(fr'E:\grant_figs_202604\{animal_name},{animal_name}{day}{subfile}.png')
        plt.savefig(fr'E:\grant_figs_202604\{animal_name},{animal_name}{day}{subfile}.svg')
        ...
        plt.figure()
        plt.title(self.animal)
        plt.imshow(np.where((self.amp_mask & self.mask), sign_map, np.nan),  cmap=pl.cm.jet) # only plot where amplitude map exceeds threshoold
        # plt.imshow(np.where((self.mask), sign_map, np.nan), cmap=pl.cm.jet)
        plt.show()
        plt.savefig(fr'E:\grant_figs_202604\{self.animal}_{self.day} masked sign map.png')
        plt.savefig(fr'E:\grant_figs_202604\{self.animal}_{self.day} masked sign map.svg')

        # fig, ax = plt.subplots(1,2, figsize = (13,4))
        # plt.title(self.animal)
        # e_contour = ax[0].contourf(np.where((self.amp_mask & self.mask), elevation_map, np.nan), origin = 'upper', aspect = 'equal', cmap=pl.cm.jet, levels=20, zorder=-1)
        # # self.ax[0].imshow(np.where((self.amp_mask & self.mask), self.patch, np.nan), cmap=pl.cm.gray, norm=norm)
        # cbar = pl.colorbar(e_contour, ax=ax[0], fraction=0.04)
        # tick_vals = cbar.get_ticks()
        # cbar.set_ticks(tick_vals)
        # cbar.set_ticklabels(np.round(tick_vals * deg_per_phase_height, 2))  # Convert to degrees
        # self.ax[0].set_title('Elevation Contours')
        # self.ax[0].set_xticks([])
        # self.ax[0].set_yticks([])
        # a_contour = ax[1].contourf(np.where((self.amp_mask & self.mask), azimuth_map, np.nan),origin = 'upper',  aspect = 'equal',cmap=pl.cm.jet, levels=20, zorder=-1)
        # cbar = pl.colorbar(a_contour, ax=ax[1], fraction=0.04)
        # tick_vals = cbar.get_ticks()
        # cbar.set_ticks(tick_vals)
        # cbar.set_ticklabels(np.round(tick_vals * deg_per_phase_width, 2))  # Convert to degrees
        # self.ax[1].set_title('Azimuth Contours')
        # self.ax[1].set_xticks([])
        # self.ax[1].set_yticks([])
        # plt.show()
        # plt.savefig(fr'E:\grant_figs_202604\{self.animal}{self.day} contours.png')
        # plt.savefig(fr'E:\grant_figs_202604\{self.animal}{self.day} contours.svg')

        return self.fig, self.ax
    def plot_raw(self):
        '''Plot raw phase and amplitude maps'''
        self.figraw, self.axraw = pl.subplots(nrows=1, ncols=2, figsize=(13, 4))
        # Plot amplitude map
        ...
        # np.where((self.amp_mask & self.mask), self.amp1, np.nan),
        max_val = max(np.max(self.amp1), np.max(self.amp2))
        amp_az = self.axraw[0].imshow(np.where((self.amp_mask & self.mask), self.amp1, np.nan), aspect = 'equal', cmap=pl.cm.jet, vmin = 0, vmax = max_val)
        self.axraw[0].set_title('Raw amplitude azimuth')
        cbar = pl.colorbar(amp_az, ax= self.axraw[0], fraction=0.04)

        amp_el = self.axraw[1].imshow(np.where((self.amp_mask & self.mask), self.amp2, np.nan),  aspect = 'equal',cmap=pl.cm.jet, vmin = 0, vmax = max_val)
        self.axraw[1].set_title('Raw amplitude elevation')
        cbar = pl.colorbar(amp_el, ax= self.axraw[1], fraction=0.04)
        ...
        plt.savefig(fr'E:\grant_figs_202604\{self.animal}_{self.day} raw phase and amp.png')
        plt.savefig(fr'E:\grant_figs_202604\{self.animal}_{self.day} raw phase and amp.svg')
...
def run_retmap(config_path, mode, min_area = 100, animal_dob=None,animal=None, day=None, draw_mask = True, draw_lines = True):
    ...
    # new_config_path = Path(r"F:\Erica\retmap") / Path(config_path).parents[3].name / Path(config_path).parents[2].name / \
    #                   Path(config_path).parents[1].name / Path(config_path).name
    # new_config_path = Path(r"F:\Erica\retmap") / Path(config_path).parts[-5] / Path(config_path).parts[-4] / \
    #            Path(config_path).parts[-3]
    # print(new_config_path)

    params = load_parameters(config_path)

    # print(Path(config_path).drive)
    ...
    params['savefolder'] = str(Path(config_path).parents[0]) # now save folder points to: 'drive:\dark_drift\data\animal\day\retmap\wdf'
    params['inputfolder'] =  str(Path(config_path).parents[0])
    # print(params['inputfolder'])
    print('Running Retinotopic_mapping.py %s\t' % (version), datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
...
# from matplotlib import patches
#
# def to_uint8_video(arr, p_lo=1, p_hi=99):
#     arr = np.asarray(arr)
#     vmin, vmax = np.nanpercentile(arr, [p_lo, p_hi])
...
#
#
# def load_mj2_opencv(path, start=0, n_frames=None, gray=True):
#     cap = cv2.VideoCapture(path)
#     if not cap.isOpened():
#         raise RuntimeError(f"Could not open video: {path}")
#
#     # jump to a starting frame (frame-accurate for many files)
#     cap.set(cv2.CAP_PROP_POS_FRAMES, start)
#
#     frames = []
#     read = 0
#     while True:
#         if n_frames is not None and read >= n_frames:
#             break
#         ret, frame = cap.read()
#         if not ret:
#             break
#         if gray:
#             frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # (H,W) uint8
#         frames.append(frame)
#         read += 1
#
#     cap.release()
#     return np.stack(frames, axis=0)  # (T,H,W) if gray=True
# def fit_circle(xy):
...
#         num_frames = 0
#     else:
#         num_frames = end_frame - start_frame # this isnt used. ERROR?????
#
...
#     config = ConfigParser()
#     f = open(os.path.join(fn, 'config.txt'), 'r')
#     config.read_file(f)
#     params = {'inputfolder': fn,
#               'savefolder': fn,
...
#         # gui.show()
#
#         self.fig, self.ax = pl.subplots(nrows=2, ncols=2, figsize=(12, 7))
...
#         pl.suptitle(f'{animal_name}, {day}, {subfile}' )
#
#         # if not os.path.exists(os.path.join(self.savefoldere)):
#         #     os.makedirs(os.path.join(self.savefolder))
#         plt.savefig(fr'{self.savefolder}\{animal_name}{day}{subfile}.png')
#         plt.savefig(fr'{self.savefolder}\{animal_name}{day}{subfile}.svg')
#
#         fig, ax = plt.subplots (figsize = (7,7))
#         ax.imshow(np.where(self.mask, rotate_image(self.ref, self.rotateMap), np.nan), cmap=pl.cm.gray, vmin=0) # plot image
#         ax.imshow(np.where(self.mask,rotate_image(self.patch, self.rotateMap), np.nan), cmap=pl.cm.hsv) # plot contours
#         # ax.set_title('Overlay')
#         ax.set_xticks([])
#         ax.set_yticks([])
#         plt.savefig(fr'{self.savefolder}\overlay {animal_name}{day}{subfile}.png')
#         plt.savefig(fr'{self.savefolder}\overlay {animal_name}{day}{subfile}.svg')
#
#         return self.fig, self.ax
#     def plot_raw(self):
#         '''Plot raw phase and amplitude maps'''
...
#         evt = sbx_get_ttlevents(fn + self.evt_suffix)
#         start_frame = evt[evt > 0][0] - 1  # -1 to start counting from 0
#         print(start_frame) #108
#         print(nimagtot) #1700
#         print(fn)
#         data_ds = load_data(fn, start_frame=start_frame, end_frame=nimagtot, resize=1) #shape n_frames (1700), y, x
#         # data_ds = load_data(fn, start_frame=start_frame, end_frame=nimagtot, resize=None)  # shape n_frames (1700), y, x
#         print(data_ds.shape)
#
#         # # usage
#         vid = load_mj2_opencv(fn, start=start_frame, n_frames=nimagtot, gray=True)
#         vmin, vmax = np.nanpercentile(vid, [1, 99])
#         vid8 = np.clip((vid - vmin) / (vmax - vmin + 1e-8), 0, 1)
#         whole_vid8 = (vid8 * 255).astype(np.uint8)
#
#         average_vid = vid.reshape(self.nrep, -1, vid.shape[-2], vid.shape[-1]).mean(axis = 0) #10 repeats
#         vmin, vmax = np.nanpercentile(average_vid, [1, 99])
#         vid8 = np.clip((average_vid - vmin) / (vmax - vmin + 1e-8), 0, 1)
#         vid8 = (vid8 * 255).astype(np.uint8)
#
#         iio.imwrite(os.path.join(self.savefolder, f'whole_vid_{Path(fn).name}.mp4'), whole_vid8, fps=30)
#         iio.imwrite(os.path.join(self.savefolder, f'vid_{Path(fn).name}.mp4'), vid8, fps=30)
#
#         #commenting this out while making video
#         # # Save a copy of the background image
#         # self._reference = np.nanmean(data_ds[0:10], axis=0)
#         # end = timer()
#         # print('File loaded - %.2f s' % (end - start))
...
#         # Perform Fourier analysis
#         data_a = [self._get_phase_map(fn, 'a') for fn in self.filepath_a]
#         data_e = [self._get_phase_map(fn, 'e') for fn in self.filepath_e] # commenting this out just while im trying to generate my videp
#
#         # commenting this out just while im trying to generate my videp
#         #
#         # # Combine phase maps
#         # if len(data_a) == 2:
#         #     phase_map_a, amp_map_a = self._combine_maps(data_a[0], data_a[1])
